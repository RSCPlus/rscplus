...
public class StreamUtil {
	...
	public static void initializeStream(String address, int port) {
		if (Reflection.createSocket == null || Reflection.stream == null || Reflection.clientStreamField == null) return;
		
		try {
			Socket connectionSocket = (Socket)Reflection.createSocket.invoke(Client.instance, -12, port, address);
	...
	public static void xteaEncrypt(Object buffer, int xtea_start, int[] keys) {
		...
		try {
			Reflection.xteaEncrypt.invoke(buffer, (byte)87, xtea_start, keys, StreamUtil.getBufferOffset(buffer));
	    } catch (Exception e) {
	    }
	}
