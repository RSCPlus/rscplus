...
public class Panel {
  ...
  public static void draw_extra_hook(Object panelSource, int controlId, int controlType) {
    ...
    try {
      if (controlType == 15) {
        /**
         * DEPRECTATED - mudclient already has built in rendering method for checkbox, using control
         * type 14 If mudclient didn't have a render method for checkbox the below would be needed.
         * Instead can use as POC on how to render a new element if there is a need (though very
         * unlikely since probably old panel element renderings from earlier mudclients are still
         * there)
         */
        ...
        // Panel.drawCheckBox(panelSource, controlId, x[controlId], y[controlId], width[controlId],
        // height[controlId]);
      }
    } catch (Exception e) {
    }
  ...
  public static void drawCheckBox(Object panelSource, int id, int x, int y, int w, int h) {
    if (Reflection.menuRenderer == null
        || Reflection.drawBox == null
        || Reflection.drawLineHoriz == null
        || Reflection.drawLineVert == null) return;
  ...
  public static int addCenterTextTo(
    ...
    try {
      count =
          (int)
              Reflection.addCenterText.invoke(
                  panelSource, isBackground, (byte) -126, fontSize, xPos, textString, yPos);
    } catch (Exception e) {
    }
