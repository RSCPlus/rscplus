...
public class Bank {
  ...
  private static final int[] runesWeaponsArmourSorted = {
  ...
  private static final int[] foodPotionsSorted = {
  ...
  private static final int[] toolsResourcesSorted = {
  ...
  private static final int[] itemsAlphabetical = {
  ...
  private static final int[] mostValuableItems = {
  ...
  private static final int[] typeSortNice = {
  ...
  // item metadata
  private static final int[] itemValues = {
  ...
  public static boolean processPacket(int opcode, int psize) {
    ...
    // TODO - place to appropriate setting to define if we are having "custom" bank features
    if (true && (opcode == 42 || opcode == 249)) {
      if (opcode == 42) {
        try {
        ...
        Client.new_count_items_bank = StreamUtil.getUnsignedByte(Client.packetsIncoming) & 0xFF;
        Client.bank_items_max = StreamUtil.getUnsignedByte(Client.packetsIncoming) & 0xFF;

        for (int index = 0; index < Client.new_count_items_bank; ++index) {
          Client.new_bank_items[index] =
              StreamUtil.getUnsignedShort(Client.packetsIncoming) & 0xFFFF;
          Client.new_bank_items_count[index] = StreamUtil.getUnsignedInt3(Client.packetsIncoming);
        }

        try {
          // call to original updateBank
        ...
        if (itemCount == 0) {
          --Client.new_count_items_bank;

          for (int index = slot; Client.new_count_items_bank > index; ++index) {
            Client.new_bank_items[index] = Client.new_bank_items[index + 1];
            Client.new_bank_items_count[index] = Client.new_bank_items_count[1 + index];
          }
        } else {
          Client.new_bank_items[slot] = item;
          Client.new_bank_items_count[slot] = itemCount;
          if (slot >= Client.new_count_items_bank) {
            Client.new_count_items_bank = 1 + slot;
          }
        }
  ...
  private static void sortBank() {
    ...
    if (buttonActive[6]) {
      // release date sort
      // TODO: this is technically just item ID sort. A few exceptions do exist to the Item ID =
      // Release Date rule.
      ...
      for (int valueIdx = 0; valueIdx < mostValuableItems.length; valueIdx++) {
        for (int i = 0; i < 255; i++) {
          if (Client.new_bank_items[i] == mostValuableItems[valueIdx]
              && Client.new_bank_items_count[i] != 0) {
            System.out.print(Client.new_bank_items[i] + ", ");
            tmpNewBankItems[idx] = Client.new_bank_items[i];
            tmpNewBankItemsCount[idx] = Client.new_bank_items_count[i];
      ...
      for (int valueIdx = 0; valueIdx < itemsAlphabetical.length; valueIdx++) {
        for (int i = 0; i < 255; i++) {
          if (Client.new_bank_items[i] == itemsAlphabetical[valueIdx]
              && Client.new_bank_items_count[i] != 0) {
            System.out.print(Client.new_bank_items[i] + ", ");
            tmpNewBankItems[idx] = Client.new_bank_items[i];
            tmpNewBankItemsCount[idx] = Client.new_bank_items_count[i];
      ...
      for (int itemId = 0; itemId < 1290; itemId++) {
        for (int i = 0; i < 255; i++) {
          if (Client.new_bank_items[i] == itemId
              && Client.new_bank_items_count[i] != 0
    ...
    if (search.trim().equals("") || help) {
      ...
      if (!Client.show_bank) {
        ...
        if (search.trim().equals("reset")) {
          // resetSearch();
          Client.displayMessage("@whi@Reset bank search not implemented lol.", Client.CHAT_QUEST);
        } else {
    ...
    for (int i = 0; i < tmpNewBankCountItems; i++) {
      ...
      if (buttonActive[4]) { // search term
        if (Settings.START_SEARCHEDBANK.get("custom") && !search.equals("")) {
          shouldInclude |=
              Item.item_name[tmpNewBankItems[i]].toLowerCase().contains(search.toLowerCase());
          ++filterCount;
        }
      }
  ...
  public static void drawBankAugmentations(Graphics2D g2) {
    if (Client.show_bank) {
      ...
      int y =
          heightFromTop
              + 12; // bank authentically starts render 12 px down TODO: adjust once bank is
                    // resizable
