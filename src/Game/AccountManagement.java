...
public class AccountManagement {
  ...
  public static String recoveryQuestions[] =
      new String[] {
        "Where were you born?",
  ...
  public static void register(String user, String pass) {
    if (Client.login_delay > 0) {
      ...
      try {
        ...
        /**
         * If we wanted a RSC175 would need to get added just below a putShort based on refererid /
         * limit30
         */
        ...
        if (response == 2) {
          ...
          Client.setResponseMessage(
              "Wait 60 seconds then retry", "That username is already in use.");
        } else if (response == 5) {
          ...
          Client.setResponseMessage(
              "for cheating or abuse", "Account has been temporarily disabled");
        } else if (response == 12) {
          Client.setResponseMessage(
              "for cheating or abuse", "Account has been permanently disabled");
        } else if (response == 13) {
          Client.setResponseMessage("Please choose another username", "Username already taken.");
        } else if (response == 14) {
          Client.setResponseMessage(
              "Please try again later", "Sorry! The server is currently full.");
  ...
  public static void forgotPass(String user) {
    ...
    try {
      ...
      Panel.setControlText(
          Client.panelRecovery,
          Client.controlRecoveryTop,
          "@yel@To prove this is your account please provide the answers to");
      Panel.setControlText(
          Client.panelRecovery,
          Client.controlRecoveryBottom,
  ...
  public static void recover() {
    ...
    try {
      ...
      String oldPass =
          Client.formatText(
              Panel.getControlText(Client.panelRecovery, Client.recoverOldPassInput), 20);
      String newPass =
          Client.formatText(
              Panel.getControlText(Client.panelRecovery, Client.recoverNewPassInput), 20);

      StreamUtil.newPacket(8);
      Object buffer = StreamUtil.getStreamBuffer();
      StreamUtil.putLongTo(
          buffer,
          Util.username2hash(Panel.getControlText(Client.panelLogin, Client.loginUserInput)));
      ...
      for (int i = 0; i < 5; ++i) {
        /**
         * In RSC127 it would have sent an encrypted "hash" like of answers, but seems in later
         * revisions incl. up to RSC175 was changed to just send the encrypted answers, probably due
         * to collisions in RSC127 hasher function
         */
  ...
  public static void sendRecoveryQuestions() {
    ...
    for (int idx = 0; idx < 5; ++idx) {
      ...
      String answer =
          Panel.getControlText(Client.panelRecoveryQuestions, Client.controlAnswerInput[idx]);
      answer = Util.formatString(answer, 50);
  ...
  public static void sendContactDetails(
      String fullName, String zipCode, String country, String email) {
    ...
    if (Settings.SHOW_ACCOUNT_SECURITY_SETTINGS.get(Settings.currentProfile)
      ...
      if (opcode
        ...
        for (int questionIndex = 0; questionIndex < 5; ++questionIndex) {
          ...
          Client.controlRecoveryText[questionIndex] =
              recoveryQuestions[recoveryIndices[questionIndex]];
          Panel.setControlText(
              Client.panelRecoveryQuestions, Client.controlAnswerInput[questionIndex], "");
          Panel.setControlText(
              Client.panelRecoveryQuestions,
              Client.controlRecoveryIns[questionIndex],
              questionIndex + 1 + ": " + Client.controlRecoveryText[questionIndex]);
        }
        processed = true;
      } else if (opcode
          == 232) { // Opcode is intended to be RSC175 compatible. Not present in RSC127
  ...
  public static void panel_welcome_hook(int n) {
    try {
      ...
      if (Settings.SHOW_ACCOUNT_SECURITY_SETTINGS.get(Settings.currentProfile)) {
        ...
        // Hide the signup button when no world is configured
        if (Settings.noWorldsConfigured
            || Settings.WORLD_URLS.get(Settings.WORLD.get(Settings.currentProfile)).isEmpty()) {
  ...
  /**
   * Sets visibility of the "Signup" button from the welcome panel
   *
   * @param visible {@code boolean} value indicating desired visibility
   */
  public static void setSignupButtonVisibility(boolean visible) {
    if (Settings.SHOW_ACCOUNT_SECURITY_SETTINGS.get(Settings.currentProfile)) {
      Object panelWelcome = Client.panelWelcome;
      if (panelWelcome != null && Reflection.menuShown != null) {
        try {
          boolean[] shown = (boolean[]) Reflection.menuShown.get(panelWelcome);
          shown[2] = visible;
          shown[3] = visible;
          shown[4] = visible;
          Reflection.menuShown.set(panelWelcome, shown);
        } catch (Exception e) {
          e.printStackTrace();
        }
      }
  ...
  public static void create_account_recovery() {
    try {
      ...
      for (int i = 0; i < 5; ++i) {
        ...
        Client.controlRecoveryQuestion[i] =
            Panel.addCenterTextTo(
                Client.panelRecovery, 256, yPos - 7, i + 1 + ": question?", 1, true);
        Client.controlRecoveryInput[i] =
            Panel.addInputTo(Client.panelRecovery, 256, yPos + 7, 310, 30, 1, 80, true, true);
        yPos += 35;
      }
      ...
      Client.recoverNewPassInput =
          Panel.addInputTo(Client.panelRecovery, 146, yPos + 7, 200, 30, 1, 80, true, true);
      ...
      Client.recoverConfirmPassInput =
          Panel.addInputTo(Client.panelRecovery, 366, yPos + 7, 200, 30, 1, 80, true, true);
      yPos += 35;
      ...
      Client.chooseSubmitRecoveryButton =
          Panel.addButtonTo(Client.panelRecovery, 201, yPos, 100, 30);
      ...
      Client.chooseCancelRecoveryButton =
          Panel.addButtonTo(Client.panelRecovery, 311, yPos, 100, 30);
    } catch (Exception e) {
      e.printStackTrace();
    }
  }
  ...
  public static void create_recovery_questions() {
    try {
      ...
      int yPos = 8;
      Client.controlRecoveryQuestions =
          Panel.addCenterTextTo(
              Client.panelRecoveryQuestions,
              256,
              yPos,
      ...
      for (int questionIndex = 0; questionIndex < 5; ++questionIndex) {
        ...
        Client.controlRecoveryIns[questionIndex] =
            Panel.addCenterTextTo(
                ...
                170,
                yPos - 7,
                questionIndex + 1 + ": " + recoveryQuestions[recoveryIndices[questionIndex]],
                1,
                true);
        Client.controlAnswerInput[questionIndex] =
            Panel.addInputTo(
                Client.panelRecoveryQuestions, 170, yPos + 7, 310, 30, 1, 80, false, true);
        Panel.addButtonBackTo(Client.panelRecoveryQuestions, 370, yPos, 80, 30);
        Panel.addCenterTextTo(Client.panelRecoveryQuestions, 370, yPos - 7, "Different", 1, true);
        Panel.addCenterTextTo(Client.panelRecoveryQuestions, 370, yPos + 7, "Question", 1, true);
        Client.controlQuestion[questionIndex] =
            Panel.addButtonTo(Client.panelRecoveryQuestions, 370, yPos, 80, 30);
        Panel.addButtonBackTo(Client.panelRecoveryQuestions, 455, yPos, 80, 30);
        Panel.addCenterTextTo(Client.panelRecoveryQuestions, 455, yPos - 7, "Enter own", 1, true);
        Panel.addCenterTextTo(Client.panelRecoveryQuestions, 455, yPos + 7, "Question", 1, true);
        Client.controlCustomQuestion[questionIndex] =
            Panel.addButtonTo(Client.panelRecoveryQuestions, 455, yPos, 80, 30);
        yPos += 35;
      }
      ...
      Client.chooseFinishSetRecoveryButton =
          Panel.addButtonTo(Client.panelRecoveryQuestions, 256, yPos, 250, 30);
  ...
  public static void create_contact_details() {
    try {
      ...
      Client.controlContactDetails =
          Panel.addCenterTextTo(
              Client.panelContactDetails,
              256,
              yPos,
              "@yel@Please supply your contact details",
              5,
              true);
      yPos += 30;
      Panel.addCenterTextTo(
          ...
          256,
          yPos,
          "We need this information to provide an efficient customer support service ",
          1,
          true);
      yPos += 15;
      Panel.addCenterTextTo(
          ...
          256,
          yPos,
          "and also to work out where to locate future RuneScape servers.",
          1,
          true);
      yPos += 25;
      Panel.addCenterTextTo(
          ...
          256,
          yPos,
          "We know some people are concerned about entering their email address on",
          1,
          true);
      yPos += 15;
      Panel.addCenterTextTo(
          ...
          255,
          yPos,
          "websites, and for this reason we take our users privacy very seriously.",
          1,
          true);
      yPos += 15;
      Panel.addCenterTextTo(
          ...
          256,
          yPos,
          "For our full policy please click the relevant link below this game window",
          1,
          true);
      yPos += 40;
      Panel.addButtonBackTo(Client.panelContactDetails, xPos, yPos, boxWidth, 30);
      Panel.addCenterTextTo(Client.panelContactDetails, xPos, yPos - 7, "Full name", 1, true);
      Client.fullNameInput =
          Panel.addInputTo(
              Client.panelContactDetails, xPos, yPos + 7, boxWidth, 30, 1, 80, false, true);
      yPos += 35;
      Panel.addButtonBackTo(Client.panelContactDetails, xPos, yPos, boxWidth, 30);
      Panel.addCenterTextTo(
      ...
      yPos += 35;
      Panel.addButtonBackTo(Client.panelContactDetails, xPos, yPos, boxWidth, 30);
      Panel.addCenterTextTo(Client.panelContactDetails, xPos, yPos - 7, "Country", 1, true);
      Client.countryInput =
          Panel.addInputTo(
              Client.panelContactDetails, xPos, yPos + 7, boxWidth, 30, 1, 80, false, true);
      yPos += 35;
      Panel.addButtonBackTo(Client.panelContactDetails, xPos, yPos, boxWidth, 30);
      Panel.addCenterTextTo(Client.panelContactDetails, xPos, yPos - 7, "Email address", 1, true);
      Client.emailInput =
          Panel.addInputTo(
              Client.panelContactDetails, xPos, yPos + 7, boxWidth, 30, 1, 80, false, true);
      yPos += 35;
      Panel.addButtonBackTo(Client.panelContactDetails, xPos, yPos, 100, 30);
      Panel.addCenterTextTo(Client.panelContactDetails, xPos, yPos, "Submit", 4, true);
      Client.chooseSubmitContactDetailsButton =
          Panel.addButtonTo(Client.panelContactDetails, xPos, yPos, 100, 30);
  ...
  public static void drawChangePassInput(int mouseX, int mouseY, int mouseButtonClick) {
    ...
    if (panelPasswordChangeMode == 6) {
      Renderer.drawStringCenter(
          "Please enter your current password", Renderer.width / 2, yPos, 4, 0xFFFFFF);
      yPos += 25;
      displayString = "*";
      ...
      Renderer.drawStringCenter(
          "Please enter your new password", Renderer.width / 2, yPos, 4, 0xFFFFFF);
      yPos += 25;
      displayString = "*";
      ...
      if (Client.modal_text.length() > 0) {
        ...
        if (newPassword
            .trim()
            .equalsIgnoreCase(
                Panel.getControlText(Client.panelLogin, Client.loginUserInput).trim())) {
      ...
      Renderer.drawStringCenter(
          "Enter password again to confirm", Renderer.width / 2, yPos, 4, 0xFFFFFF);
      yPos += 25;
      displayString = "*";
      ...
      if (panelPasswordChangeMode == 4) {
        Renderer.drawStringCenter(
            "Ok, your request has been sent", Renderer.width / 2, yPos, 4, 0xFFFFFF);
        yPos += 25;
        Renderer.drawStringCenter("Press any key to close", Renderer.width / 2, yPos, 4, 0xFFFFFF);
        return;
      }
      ...
      if (panelPasswordChangeMode == 7) {
        Renderer.drawStringCenter(
            "Your password must not be", Renderer.width / 2, yPos, 4, 0xFFFFFF);
        yPos += 25;
        Renderer.drawStringCenter(
  ...
  public static String[][] originalSecurityTips = {
    {
    ...
    },
    {
      ...
      "These programs WILL steal your password."
    },
    {
  ...
  public static int welcome_security_tip_day_hook(int xPos, int yPos) {
    ...
    if (Settings.SHOW_SECURITY_TIP_DAY.get(Settings.currentProfile)) {
      ...
      currYPos += 15;
      currYPos = drawTip(originalSecurityTips[Client.tipOfDay], currYPos);
      currYPos += 22;
    }
    return currYPos - yPos;
  }

  public static int drawTip(String[] tipLines, int yPos) {
    for (int i = 0; i < tipLines.length; i++) {
      Renderer.drawStringCenter(tipLines[i], Renderer.width / 2, yPos, 1, 0xFFFFFF);
      yPos += 15;
    }
    return yPos;
  }
    ...
    try {
        ...
        if (Panel.isSelected(Client.panelRegister, Client.chooseSubmitRegisterButton)) {
          if (Panel.getControlText(Client.panelRegister, Client.chooseUserInput) != null
            ...
            if (Panel.getControlText(Client.panelRegister, Client.choosePasswordInput)
                .trim()
                .equalsIgnoreCase(
                    Panel.getControlText(Client.panelRegister, Client.chooseUserInput).trim())) {
              Panel.setControlText(
                  Client.panelRegister,
                  Client.controlRegister,
                  "@yel@Your password must not be the same as your username!");
              return;
            }
      ...
      if (Client.login_screen == Client.SCREEN_PASSWORD_RECOVERY) {
        ...
        if (Panel.isSelected(Client.panelRecovery, Client.chooseSubmitRecoveryButton)) {
          ...
          String confirmPass =
              Panel.getControlText(Client.panelRecovery, Client.recoverConfirmPassInput);
    ...
    try {
      if (customQuestionEntry != -1) {
        if (Client.pm_text.length() > 0) {
          ...
          Panel.setControlText(
              Client.panelRecoveryQuestions,
              Client.controlRecoveryIns[customQuestionEntry],
              customQuestionEntry + 1 + ": " + Client.controlRecoveryText[customQuestionEntry]);
          Panel.setControlText(
        ...
        for (int questionIndex = 0; questionIndex < 5; ++questionIndex) {
          if (Panel.isSelected(
              Client.panelRecoveryQuestions, Client.controlQuestion[questionIndex])) {
            boolean hasDistinctQuestions = false;

            while (!hasDistinctQuestions) {
              recoveryIndices[questionIndex] =
                  (recoveryIndices[questionIndex] + 1) % recoveryQuestions.length;
              hasDistinctQuestions = true;

              for (cursorIndex = 0; cursorIndex < 5; ++cursorIndex) {
                if (cursorIndex != questionIndex
                    && recoveryIndices[cursorIndex] == recoveryIndices[questionIndex]) {
        ...
        if (Panel.isSelected(Client.panelRecoveryQuestions, Client.chooseFinishSetRecoveryButton)) {
          ...
          while (true) {
            if (cursorIndex >= 5) {
              for (int outerIdx = 0; outerIdx < 5; ++outerIdx) {
                String checkedAnswer =
                    Panel.getControlText(
                        Client.panelRecoveryQuestions, Client.controlAnswerInput[outerIdx]);

                for (int innerIdx = 0; innerIdx < outerIdx; ++innerIdx) {
                  String questionAnswer =
                      Panel.getControlText(
                          Client.panelRecoveryQuestions, Client.controlAnswerInput[innerIdx]);
            ...
            String chkAnswer =
                Panel.getControlText(
                    Client.panelRecoveryQuestions, Client.controlAnswerInput[cursorIndex]);
            if (chkAnswer == null || chkAnswer.length() < 3) {
              Panel.setControlText(
                  Client.panelRecoveryQuestions,
                  Client.controlRecoveryQuestions,
                  "@yel@Please provide a longer answer to question: " + (cursorIndex + 1));
              return;
            }
  ...
  public static void contact_details_input(int n1, int mouseY, int n3, int mouseX) {
    try {
      ...
      if (name != null
          ...
          && email.length() != 0) {
        sendContactDetails(
            name, zipcode, country, email); // this generates & sends RSC175 opcode 253
        Client.clearScreen();
      ...
      Panel.setControlText(
          Client.panelContactDetails,
          Client.controlContactDetails,
          "@yel@Please fill in all the requested details");
    } catch (Exception e) {
      e.printStackTrace();
    }
  }
