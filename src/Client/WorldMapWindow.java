...
public class WorldMapWindow {
    ...
    private static void initialize() {
        ...
        for (int i = 0; i < 4; i++)
            mapImageBuffer[i] = graphicsConfiguration.createCompatibleImage(2448 + 1, 2736 + 1, Transparency.OPAQUE); // + 1 coords for border
        ...
        synchronized (renderLock) {
            ...
            if (showScenery) {
                for (MapScenery scenery : mapSceneries) {
                    ...
                    if (scenery.isSearchable() && searchText.length() > 0 && (searchNumber == scenery.id || scenery.searchName.toLowerCase().contains(searchText.toLowerCase())))
                        g.setColor(Renderer.color_item_highlighted);
                    else if (p.height == 0 && (scenery.id == 0 || scenery.id == 1))
                        g.setColor(color_scenery_tree);
                    else if (p.height == 1 && (scenery.id == 70 || scenery.id == 205 || scenery.id == 38 || scenery.id == 4 || scenery.id == 208 || scenery.id == 108))
                        g.setColor(color_scenery_deadtree);
                    else if (plane == 0 || plane == 3)
                        g.setColor(color_scenery_normal);
                    else
                        g.setColor(color_scenery_indoors);
            ...
            if (plane == 0) {
                if (showIcons) {
                    for (MapGlyph glyph : mapGlyphs) {
                        ...
                        if (searchText.length() > 0 && legendText[glyph.id].toLowerCase().contains(searchText.toLowerCase()))
                        ...
                        g.drawImage(legends[glyph.id], glyphX + (7 - legends[glyph.id].getWidth(null) / 2), glyphY + (7 - legends[glyph.id].getHeight(null) / 2), null);
                    }
                }

                if (showLabels) {
                    for (MapLabel label : mapLabels)
                        drawMapLabel(g, label.x, label.y, label);
                }
            }
    ...
    private static void drawButton(Graphics2D g, String text, Rectangle bounds) {
        ...
        Renderer.drawShadowText(g, text, bounds.x + (bounds.width / 2), bounds.y + (bounds.height / 2), Renderer.color_text, true);
    }
    ...
    private static void buildSearchResults() {
        ...
        for (SearchResult glyph : mapGlyphs) {
            if (glyph.isSearchable() && glyph.getSearchName().toLowerCase().contains(searchText.toLowerCase())) {
                results.add(glyph);
        ...
        for (SearchResult label : mapLabels) {
            if (label.isSearchable() && label.getSearchName().toLowerCase().contains(searchText.toLowerCase())) {
                results.add(label);
                if (results.size() > SEARCH_RESULTS_LIMIT) {
                    searchOverflow = true;
                    searchResults = null;
        ...
        for (SearchResult scenery : mapSceneries) {
            ...
            if (scenery.isSearchable() && (sceneryObj.id == searchNumber || scenery.getSearchName().toLowerCase().contains(searchText.toLowerCase()))) {
                results.add(scenery);
                if (results.size() > SEARCH_RESULTS_LIMIT) {
                    searchOverflow = true;
                    searchResults = null;
    ...
    private static void drawSearch(Graphics2D g, String text, Rectangle bounds) {
        ...
        setAlpha(g, 1.0f);
        g.drawRect(bounds.x, bounds.y, bounds.width, bounds.height);
        Renderer.drawShadowText(g, searchTerm, bounds.x + BORDER_SIZE, bounds.y + (bounds.height / 2) + 4, searchColor, false);

        if (searchResults != null || searchOverflow) {
            // Calculate height
            int searchHeight;
            if (searchOverflow || searchResults.length == 0)
                searchHeight = 16 + BORDER_SIZE;
            else if (searchResults.length <= SEARCH_RESULTS_MAX)
                searchHeight = searchResults.length * 16 + BORDER_SIZE;
            else
                searchHeight = (SEARCH_RESULTS_MAX + 1) * 16 + BORDER_SIZE;
            ...
            for (int i = 0; i < SEARCH_RESULTS_MAX; i++) {
                ...
                Renderer.drawShadowText(g, resultName, bounds.x + BORDER_SIZE, bounds.y - 8 - (i * 16), resultColor, false);
            ...
            if (searchOverflow) {
                Renderer.drawShadowText(g, "Too many results, narrow your search", bounds.x + BORDER_SIZE, bounds.y - 8, Renderer.color_text, false);
            } else if (searchResults.length == 0) {
                Renderer.drawShadowText(g, "No matches found", bounds.x + BORDER_SIZE, bounds.y - 8, Renderer.color_text, false);
            } else {
                if (searchResults.length > SEARCH_RESULTS_MAX) {
                    ...
                    if (remaining == 1)
                        resultText = "result";
                    Renderer.drawShadowText(g, Integer.toString(remaining) + " more " + resultText + "...", bounds.x + BORDER_SIZE, bounds.y - 8 - (SEARCH_RESULTS_MAX * 16), Renderer.color_text, false);
                }
            }
        }
    }
    ...
    {
        if (mouse.x >= bounds.x && mouse.y >= bounds.y &&
            mouse.x <= bounds.x + bounds.width && mouse.y <= bounds.y + bounds.height)
            return true;
    ...
    public static void initAssets() {
        ...
        try {
            ...
            for (int i = 0; i < obj.length(); i++) {
                ...
                if (entry.has("bold"))
                    label.bold = entry.getBoolean("bold");
                if (entry.has("colour")) {
        ...
        try {
            ...
            for (int i = 0; i < obj.length(); i++) {
                ...
                if (type.equalsIgnoreCase("rare-trees"))
                    glyph.id = 0;
                else if (type.equalsIgnoreCase("dungeon"))
                    glyph.id = 1;
                else if (type.equalsIgnoreCase("altar"))
                    glyph.id = 2;
                else if (type.equalsIgnoreCase("amulet-shop"))
                    glyph.id = 3;
                else if (type.equalsIgnoreCase("anvil"))
                    glyph.id = 4;
                else if (type.equalsIgnoreCase("apothecary"))
                    glyph.id = 5;
                else if (type.equalsIgnoreCase("archery-shop"))
                    glyph.id = 6;
                else if (type.equalsIgnoreCase("armour-conversion"))
                    glyph.id = 7;
                else if (type.equalsIgnoreCase("axe-shop"))
                    glyph.id = 8;
                else if (type.equalsIgnoreCase("bank"))
                    glyph.id = 9;
                else if (type.equalsIgnoreCase("bed"))
                    glyph.id = 10;
                else if (type.equalsIgnoreCase("body-armour-shop"))
                    glyph.id = 11;
                else if (type.equalsIgnoreCase("certificate-trader"))
                    glyph.id = 12;
                else if (type.equalsIgnoreCase("clothes-shop"))
                    glyph.id = 13;
                else if (type.equalsIgnoreCase("combat-practice"))
                    glyph.id = 14;
                else if (type.equalsIgnoreCase("cookery-shop"))
                    glyph.id = 15;
                else if (type.equalsIgnoreCase("crafting-shop"))
                    glyph.id = 16;
                else if (type.equalsIgnoreCase("fishing-shop"))
                    glyph.id = 17;
                else if (type.equalsIgnoreCase("food-shop"))
                    glyph.id = 18;
                else if (type.equalsIgnoreCase("furnace"))
                    glyph.id = 19;
                else if (type.equalsIgnoreCase("gem-shop"))
                    glyph.id = 20;
                else if (type.equalsIgnoreCase("general-shop"))
                    glyph.id = 21;
                else if (type.equalsIgnoreCase("helmet-shop"))
                    glyph.id = 22;
                else if (type.equalsIgnoreCase("herblaw-shop"))
                    glyph.id = 23;
                else if (type.equalsIgnoreCase("jewellery-shop"))
                    glyph.id = 24;
                else if (type.equalsIgnoreCase("kebab-shop"))
                    glyph.id = 25;
                else if (type.equalsIgnoreCase("leg-armour-shop"))
                    glyph.id = 26;
                else if (type.equalsIgnoreCase("mace-shop"))
                    glyph.id = 27;
                else if (type.equalsIgnoreCase("magic-shop"))
                    glyph.id = 28;
                else if (type.equalsIgnoreCase("mining-site"))
                    glyph.id = 29;
                else if (type.equalsIgnoreCase("pickable-lock"))
                    glyph.id = 30;
                else if (type.equalsIgnoreCase("pub"))
                    glyph.id = 31;
                else if (type.equalsIgnoreCase("quest"))
                    glyph.id = 32;
                else if (type.equalsIgnoreCase("scimitar-shop"))
                    glyph.id = 33;
                else if (type.equalsIgnoreCase("shield-shop"))
                    glyph.id = 34;
                else if (type.equalsIgnoreCase("silk-trader"))
                    glyph.id = 35;
                else if (type.equalsIgnoreCase("skirt-armour-shop"))
                    glyph.id = 36;
                else if (type.equalsIgnoreCase("spinning-wheel"))
                    glyph.id = 37;
                else if (type.equalsIgnoreCase("staff-shop"))
                    glyph.id = 38;
                else if (type.equalsIgnoreCase("sword-shop"))
                    glyph.id = 39;
                else if (type.equalsIgnoreCase("tannery"))
                    glyph.id = 40;
                else if (type.equalsIgnoreCase("fishing-point"))
                    glyph.id = 41;
                else
                    glyph.id = -1;
    ...
    public static void Reset() {
        ...
        playerPlane = -1;
        if (playerPlane != prevPlayerPlane)
            mapView.repaint();
    }
    ...
    public static void drawMapLabel(Graphics2D g, int x, int y, MapLabel label) {
        if (label.bold)
            g.setFont(fontsBold[label.size]);
        else
            g.setFont(fonts[label.size]);
        ...
        for (String line : label.text.split("\n")) {
            ...
            if (searchText.length() > 0 && label.text.replaceAll("\n", " ").toLowerCase().contains(searchText.toLowerCase()))
    ...
    public static class MapRenderer extends JComponent {
        ...
        protected void paintComponent(Graphics graphics){
            ...
            if (searchText.length() > 0)
                drawButton(g, "Refresh", searchRefreshBounds);
            ...
            if (showLegend) {
                ...
                offsetY = 0;
                for (int i = 0; i < legends.length; i++) {
                    g.drawImage(legends[i], x + (7 - legends[i].getWidth(null) / 2), y + offsetY + (7 - legends[i].getHeight(null) / 2), null);
                    Renderer.drawShadowText(g, legendText[i], x + 18, y + offsetY + 12, Renderer.color_text, false);
                    offsetY += 16;
                    if (offsetY >= maxY) {
                        x += 150;
                        offsetY = 0;
    ...
    public static class SearchResult {
        ...
        public String getSearchName() { return ""; }
        public String getSearchNameExtra() { return ""; }
        public int getSearchType() { return SEARCH_NONE; }

        public int getPlayerDistance() { return 0; }
